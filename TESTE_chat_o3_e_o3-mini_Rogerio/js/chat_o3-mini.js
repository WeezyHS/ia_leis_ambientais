// /TESTE_chat_o3_e_o3-mini_Rogerio/js/chat_o3-mini.js
// Versão adaptada para testar modelo o3-mini
document.addEventListener('DOMContentLoaded', () => {
    ...
    const addMessageToUI = (role, content) => {
      ...
      avatar.textContent = role === 'user' ? '👤' : '⚡'; // Emoji diferente para o3-mini
    ...
    const startNewChat = () => {
      ...
      conversations.set(tempKey, {
        id: null,
        title: 'Novo chat o3-mini',
        messages: [{ role: 'assistant', content: '⚡ Olá! Sou o modelo o3-mini da OpenAI. Como posso ajudar você hoje?' }]
      });
  
      createListItem('Novo chat o3-mini', tempKey, null);
    ...
    const handleSendMessage = async (event) => {
      ...
      if (conv.title === 'Novo chat o3-mini') {
        const short = userMessage.length > 28 ? userMessage.slice(0, 28) + '…' : userMessage;
        updateListItemTitle(currentConversationKey, short || 'Novo chat o3-mini');
      }
      ...
      try {
        ...
        const response = await fetch('/ask-ia-o3-mini', {
          method: 'POST',
        ...
        console.error('Erro ao comunicar com a IA o3-mini:', error);
        conv.messages.push({ role: 'assistant', content: 'Desculpe, ocorreu um erro ao tentar conectar-me ao modelo o3-mini.' });
        addMessageToUI('assistant', 'Desculpe, ocorreu um erro ao tentar conectar-me ao modelo o3-mini.');
      }
    };
